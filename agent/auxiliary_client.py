    ...
    *,
    ...
    if task == "vision":
        ...
        client, final_model = _get_cached_client(
            ...
            api_key=resolved_api_key,
            api_mode=resolved_api_mode,
            main_runtime=main_runtime,  # FABLE5 H4: the sync twin (:6192) and the
            # async auto path (:6811) both pass this; without it a `custom:` main
            # provider misroutes on the async path (regression of #45472).
        )
        if client is None:
            _explicit = (resolved_provider or "").strip().lower()
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:

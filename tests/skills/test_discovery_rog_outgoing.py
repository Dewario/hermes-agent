"""Synthetic tests for Slice B2 ROG draft_outgoing_request."""
...
SCRIPT = REPO / "skills" / "legal" / "discovery-workflow" / "scripts" / "rog_outgoing.py"
...
out = _load(SCRIPT, "rog_outgoing")
cg = _load(CASEGRAPH, "casegraph_for_rog_outgoing_tests")


def _matter(tmp_path: Path, matter_id: str = "SYN-OROG-A", prefix: str = "THORN-PROD") -> Path:
    ...
    (matter / "01_discovery_outgoing" / "rog_issue_brief.md").write_text(
...
def test_parse_and_draft_splits_multi_topic(tmp_path):
    matter = _matter(tmp_path)
    assert out.main(["parse-issue-brief", str(matter)]) == 0
    assert out.main(["draft-outgoing-rog", str(matter)]) == 0
    items = _read_jsonl(matter / "02_outputs" / "outgoing_rog_items.jsonl")
    assert len(items) >= 4
    ...
    assert all(i["item_id"].startswith("ORI-") for i in items)


def test_rejects_unknown_tag(tmp_path):
    matter = _matter(tmp_path)
    brief = matter / "01_discovery_outgoing" / "rog_issue_brief.md"
    brief.write_text("- [not_a_tag] State all facts concerning x.\n", encoding="utf-8")
...
def test_rejects_objection_language(tmp_path):
    ...
    brief = matter / "01_discovery_outgoing" / "rog_issue_brief.md"
    brief.write_text(
        "- [notice] State all facts concerning x, subject to plaintiff's objection on privilege grounds.\n",
        encoding="utf-8",
    )
    assert out.main(["parse-issue-brief", str(matter)]) == 2
...
def test_package_and_validate(tmp_path):
    matter = _matter(tmp_path)
    for command in (
        "parse-issue-brief", "draft-outgoing-rog", "package-outgoing-rog", "validate-outgoing-rog",
    ):
        assert out.main([command, str(matter)]) == 0
    pkg = (matter / "02_outputs" / "outgoing_rog_set.md").read_text(encoding="utf-8")
    assert "Outgoing Interrogatories" in pkg
    assert "Outgoing interrogatory" in pkg
...
def test_live_mode_enforces_ocr(tmp_path, monkeypatch):
    matter = _matter(tmp_path)
    for command in ("parse-issue-brief", "draft-outgoing-rog", "package-outgoing-rog"):
        assert out.main([command, str(matter)]) == 0
    ...
    assert out.main(["validate-outgoing-rog", str(matter)]) == 0
...
def test_isolation(tmp_path):
    a = _matter(tmp_path, "SYN-OROG-A", "THORN-PROD")
    b = _matter(tmp_path, "SYN-OROG-B", "RIVER-PROD")
    for matter in (a, b):
        for command in ("parse-issue-brief", "draft-outgoing-rog", "package-outgoing-rog"):
            assert out.main([command, str(matter)]) == 0
    a_pkg = (a / "02_outputs" / "outgoing_rog_set.md").read_text(encoding="utf-8")
    b_pkg = (b / "02_outputs" / "outgoing_rog_set.md").read_text(encoding="utf-8")

#!/usr/bin/env python3
"""Slice C2: draft_response for RFAs from an attorney answer brief (synthetic-only).
...
REQUESTS_REL = Path("02_outputs") / "draft_rfa_requests.jsonl"
BRIEF_REL = Path("02_outputs") / "rfa_answer_brief.jsonl"
ITEMS_REL = Path("02_outputs") / "draft_rfa_response_items.jsonl"
PACKAGE_REL = Path("02_outputs") / "draft_rfa_responses.md"
META_REL = Path("02_outputs") / "draft_rfa_response_meta.json"
DEFAULT_SOURCE = Path("01_discovery_served") / "rfa_set.md"
DEFAULT_BRIEF = Path("01_discovery_proposed") / "rfa_answer_brief.md"
...
BRIEF_LINE_RE = re.compile(
    r"^\s*-\s*(?:RFA\s*)?(?P<num>\d+)\s*:\s*(?P<cls>admit|deny|qualify|"
...
cg = _load_module(CASEGRAPH_SCRIPT, "legal_casegraph_rfa_resp_draft")
_ms = _load_module(MATTER_SAFETY, "matter_safety_rfa_response_draft")
jp = _load_module(LOAD_PACK_SCRIPT, "jurisdiction_load_pack_c2")
...
def parse_served_rfa(text: str) -> list[dict[str, Any]]:
    if re.search(r"Request\s+for\s+Production", text, re.IGNORECASE) and not re.search(
        ...
        raise UsageError("source looks like RFP set; refuse RFA response drafter")
    if re.search(r"\bInterrogator(?:y|ies)\b", text, re.IGNORECASE) and not re.search(
        ...
        raise UsageError("source looks like ROG set; refuse RFA response drafter")
    rows: list[dict[str, Any]] = []
    current: str | None = None
    buf: list[str] = []
    for raw in text.splitlines():
        match = RFA_HEADING_RE.match(raw.rstrip())
        if match:
            if current is not None:
                body = " ".join(buf).strip()
                if not body:
                    raise UsageError(f"RFA No. {current}: empty body")
    ...
    if current is not None:
        ...
        rows.append({"served_number": int(current), "text": body})
    if not rows:
        raise UsageError("zero RFA items parsed")
    return rows
    ...
    for lineno, raw in enumerate(text.splitlines(), 1):
        ...
        if not match:
            raise UsageError(
                ...
                "'- N: admit|deny|qualify|lack_information|object_only | narrative'"
            )
...
def _rule_ids(classification: str, available: set[str]) -> list[str]:
    candidates = {
        "admit": ["FRCP-36-a-4", "CCP-2033-210", "FRCP-36-a-1", "CCP-2033-030"],
        "deny": ["FRCP-36-a-4", "CCP-2033-210", "FRCP-36-a-1", "CCP-2033-030"],
        "qualify": ["FRCP-36-a-4", "CCP-2033-220", "FRCP-36-a-1"],
        "lack_information": ["FRCP-36-a-4", "CCP-2033-210", "FRCP-26-e"],
        "object_only": ["FRCP-36-a-5", "CCP-2033-210", "FRCP-26-b-1", "CCP-2017-010"],
    }.get(classification, ["FRCP-36-a-1", "CCP-2033-210"])
...
def draft_items(
    ...
    for i, req in enumerate(requests, 1):
        ...
        # Prefer "Admit that …" / "Deny that …" so check-isolation does not
        # treat "Admit. The June" / "Deny. Photographs" as unregistered names.
        low = narrative.lower()
        if cls == "admit":
            if low.startswith("admit that") or low.startswith("admit."):
                response_text = narrative
            else:
                response_text = f"Admit that {narrative[0].lower()}{narrative[1:]}" if narrative else "Admit."
        elif cls == "deny":
            if low.startswith("deny that") or low.startswith("deny."):
                response_text = narrative
            else:
                response_text = f"Deny that {narrative[0].lower()}{narrative[1:]}" if narrative else "Deny."
        elif cls == "qualify":
            response_text = f"Admit in part and deny in part: {narrative}"
        elif cls == "lack_information":
            response_text = (
                "Plaintiff lacks information sufficient to admit or deny after a "
...
def cmd_parse_served(args: argparse.Namespace) -> int:
    ...
    print(f"parsed {len(rows)} served RFAs for response draft -> {root / REQUESTS_REL}")
    return 0
...
def cmd_draft(args: argparse.Namespace) -> int:
    ...
    refresh_casegraph_index(root)
    print(f"drafted {len(items)} RFA responses -> {root / ITEMS_REL}")
    return 0
...
def build_package(root: Path, items: list[dict[str, Any]], meta: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "# Draft RFA Responses - DRAFT FOR ATTORNEY REVIEW",
        "",
    ...
    for item in items:
        lines.extend([
            f"## Response to Request for Admission No. {item['served_number']}",
            "",
    ...
    lines.extend([
        "## Attorney checklist",
        "",
        "- [ ] Each admit/deny checked against indexed production",
        "- [ ] Lack-of-information diligence narrative complete",
        "- [ ] No unintended admissions",
        "- [ ] Gate commands for Slice C2 exit 0",
...
def cmd_package(args: argparse.Namespace) -> int:
    ...
    refresh_casegraph_index(root)
    print(f"wrote draft RFA responses -> {path}")
    return 0
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    if not package.is_file():
        ...
        if re.search(r"\bRFA-0\d{2,}\b", text):
            errors.append("package contains Bates-colliding RFA-00N tokens")
    for item in items:
    ...
    for command in gates:
        ...
        if code != 0:
            ...
            return 1
    print("PASS: draft RFA response validation")
    return 0
...
def _create_synthetic_matter(root: Path, matter_id: str, prefix: str) -> None:
    ...
    (root / DEFAULT_BRIEF).write_text(
        "- 1: admit | the June 1, 2024 incident report is in the indexed production.\n"
        "- 2: deny | photographs of the ladder are present in the indexed production.\n"
        "- 3: lack_information | employer payroll confirmation is still required.\n",
        encoding="utf-8",
    )
...
def cmd_selftest(_args: argparse.Namespace) -> int:
    with tempfile.TemporaryDirectory(prefix="rfa-response-draft-selftest-") as tmp:
        root = Path(tmp) / "SYN-DRFA"
        _create_synthetic_matter(root, "SYN-DRFA", "THORN-PROD")
        for command in (
            ["parse-served-rfa", str(root)],
            ["parse-answer-brief", str(root)],
            ["draft-rfa-responses", str(root)],
            ["package-rfa-response-draft", str(root)],
            ["validate-rfa-response-draft", str(root)],
        ):
            code = main(command)
            if code != 0:
        ...
        if "Admit that" not in pkg or "Deny that" not in pkg:
            print("selftest failed: expected admit/deny draft language", file=sys.stderr)
            return 1
        if "RFA-001" in pkg:
            print("selftest failed: Bates-like RFA-001 in package", file=sys.stderr)
            return 1
        print("PASS: rfa-response-draft selftest")
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("parse-served-rfa", help="parse served RFA set for drafting")
    ...
    p = sub.add_parser("draft-rfa-responses", help="draft responses from brief")
    p.add_argument("matter_dir")
    p.add_argument("--allow-stub-pack", action="store_true")
    p.set_defaults(fn=cmd_draft)

    p = sub.add_parser("package-rfa-response-draft", help="write draft responses markdown")
    p.add_argument("matter_dir")
    p.set_defaults(fn=cmd_package)

    p = sub.add_parser("validate-rfa-response-draft", help="run Slice C2 validators")
    p.add_argument("matter_dir")
    p.add_argument("--skip-live-preflight", action="store_true")
    p.add_argument("--synthetic", action="store_true")
    p.set_defaults(fn=cmd_validate)

    p = sub.add_parser("selftest", help="offline synthetic C2 E2E")

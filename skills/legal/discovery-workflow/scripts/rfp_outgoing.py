...
jp = _load_aux(LOAD_PACK_SCRIPT, "jurisdiction_load_pack_rfp_outgoing")
_limits = _load_aux(LIMITS_SCRIPT, "jurisdiction_limits_rfp_outgoing")
...
def check_outgoing_rfp_limit(root: Path, item_count: int) -> list[str]:
    ...
    limit = resolve_rfp_limit(profile, set(loaded["rule_ids"]))
    if limit is None:
        return [
            "numerical_limit_none: no numerical cap applies to requests for "
            "production in this jurisdiction absent a case-specific order."
        ]
    if item_count > limit:
        return [
            f"exceeds_numerical_limit: outgoing RFP count ({item_count}) exceeds "
            f"the attorney-set limit of {limit}."
        ]
    return []
...
def cmd_validate_outgoing_rfp(args: argparse.Namespace) -> int:
    ...
    items = read_jsonl(root / ITEMS_REL)
    errors = validate_outgoing_records(targets, items)
    limit_notes = check_outgoing_rfp_limit(root, len(items))
    limit_warnings = [n for n in limit_notes if n.startswith("numerical_limit_none")]

#!/usr/bin/env python3
"""Slice B2: draft outgoing interrogatories tied to case issue tags (synthetic-only).
...
TARGETS_REL = Path("02_outputs") / "outgoing_rog_targets.jsonl"
ITEMS_REL = Path("02_outputs") / "outgoing_rog_items.jsonl"
PACKAGE_REL = Path("02_outputs") / "outgoing_rog_set.md"
META_REL = Path("02_outputs") / "outgoing_rog_meta.json"

DEFAULT_BRIEF = Path("01_discovery_outgoing") / "rog_issue_brief.md"
...
INTERROGATORY_STEM_RE = re.compile(
    r"^(state|identify|describe|list|explain|provide|set forth)\b",
    re.IGNORECASE,
)
...
def _load_casegraph():
    ...
    spec = importlib.util.spec_from_file_location("legal_casegraph_rog_out", CASEGRAPH_SCRIPT)
...
def _load_matter_safety():
    sys.dont_write_bytecode = True
    spec = importlib.util.spec_from_file_location("matter_safety_rog_outgoing", MATTER_SAFETY)
    if spec is None or spec.loader is None:
...
jp = _load_aux(LOAD_PACK_SCRIPT, "jurisdiction_load_pack_rog_outgoing")
_limits = _load_aux(LIMITS_SCRIPT, "jurisdiction_limits_rog_outgoing")
...
def check_outgoing_rog_limit(root: Path, item_count: int) -> list[str]:
    ...
    limit = resolve_rog_limit(profile, set(loaded["rule_ids"]))
    if limit is None:
        return [
            "numerical_limit_county_local: no statewide interrogatory cap applies "
            "for this jurisdiction; limit is county-local/track-dependent. "
            "Attorney must confirm the venue's local cap or set rog_limit in "
            "matter_profile."
        ]
    if item_count > limit:
        return [
            f"exceeds_numerical_limit: outgoing interrogatory count ({item_count}) "
            f"exceeds jurisdiction limit of {limit} (absent stipulation/order)."
        ]
    return []
...
def parse_issue_brief(text: str) -> list[dict[str, Any]]:
    ...
    for lineno, raw in enumerate(text.splitlines(), 1):
        ...
        if not match:
            raise UsageError(
                f"brief line {lineno}: expected '- [tag, ...] State/Identify … [| Jury: …]'"
            )
...
def _is_multi_fact(text: str) -> bool:
    if AND_SPLIT_RE.search(text):
        return True
    # Multiple independent interrogatory stems in one line
    stems = list(INTERROGATORY_STEM_RE.finditer(text))
    if len(stems) > 1:
        return True
    return False


def _normalize_interrogatory(text: str) -> str:
    cleaned = " ".join(text.split()).strip(" .")
    if INTERROGATORY_STEM_RE.match(cleaned):
        # Capitalize first letter of stem
        body = cleaned[0].upper() + cleaned[1:] if cleaned else cleaned
        return body if body.endswith("?") or body.endswith(".") else f"{body}."
    cleaned = re.sub(r"(?i)^(all\s+)?facts?\s+(concerning|regarding|about)\s+", "", cleaned)
    sentence = f"State all facts concerning {cleaned}"
    return sentence if sentence.endswith("?") or sentence.endswith(".") else f"{sentence}."
        ...
        if _is_multi_fact(fact):
            ...
            if len(parts) >= 2:
                for part in parts:
                    items.append(
                        _item_row(
                            len(items) + 1, target, _normalize_interrogatory(part),
                            tags, jury, multi_split=True,
                        )
                    )
                continue
            items.append(_item_row(
                len(items) + 1, target, _normalize_interrogatory(fact), tags, jury,
                multi_split=False, needs_attorney=True,
                notes="Multi-topic interrogatory could not be auto-split; attorney must narrow.",
            ))
            continue
        items.append(
            _item_row(
                len(items) + 1, target, _normalize_interrogatory(fact),
                tags, jury, multi_split=False,
            )
        )
    return items
...
def _item_row(
    ...
    *,
    ...
    # Avoid Bates-like IDs (PREFIX-001). Use ORI-1 style (Outgoing Interrogatory).
    item_id = f"ORI-{n}"
    note = notes or (
        f"Targets issue tag(s): {', '.join(tags)}."
        + (
            f" Jury usefulness: {jury}"
            if jury
            else " Narrow interrogatory tied to a single issue cluster."
        )
    )
...
def cmd_parse_issue_brief(args: argparse.Namespace) -> int:
    ...
    print(f"parsed {len(rows)} outgoing ROG targets -> {root / TARGETS_REL}")
    return 0


def cmd_draft_outgoing_rog(args: argparse.Namespace) -> int:
    root = matter_root(args.matter_dir)
    targets = read_jsonl(root / TARGETS_REL)
    items = draft_outgoing_items(targets)
    if not items:
        print("zero outgoing ROG items drafted", file=sys.stderr)
        return 1
    write_jsonl(output_path(root, ITEMS_REL), items)
    refresh_casegraph_index(root)
    print(f"drafted {len(items)} outgoing ROGs -> {root / ITEMS_REL}")
    return 0
...
def validate_outgoing_records(targets: list[dict[str, Any]], items: list[dict[str, Any]]) -> list[str]:
    ...
    for item in items:
        ...
        if ADMIT_RE.search(text):
            errors.append(f"{iid}: RFA-style Admit language forbidden in ROG draft")
...
def _display_item_id(item_id: str) -> str:
    match = re.fullmatch(r"ORI-(\d+)", str(item_id))
    if match:
        return f"Outgoing interrogatory {int(match.group(1))}"
    return str(item_id)
...
def build_outgoing_package(root: Path, items: list[dict[str, Any]], brief_sha: str) -> str:
    ...
    lines = [
        ...
        "",
        "# Outgoing Interrogatories - DRAFT FOR ATTORNEY REVIEW",
        "",
        ...
        "> Not a certification that these interrogatories are ready to serve.",
    ...
    lines.extend([
        ...
        "## Attorney checklist",
        "",
        "- [ ] Every interrogatory is narrow and single-topic (or explicitly approved as multi-topic)",
        ...
        "- [ ] Gate commands for Slice B2 exit 0",
...
def cmd_package_outgoing_rog(args: argparse.Namespace) -> int:
    root = matter_root(args.matter_dir)
    targets = read_jsonl(root / TARGETS_REL)
    ...
    refresh_casegraph_index(root)
    print(f"wrote outgoing ROG package -> {path}")
    return 0
...
def cmd_validate_outgoing_rog(args: argparse.Namespace) -> int:
    root = matter_root(args.matter_dir)
    targets = read_jsonl(root / TARGETS_REL)
    items = read_jsonl(root / ITEMS_REL)
    errors = validate_outgoing_records(targets, items)
    limit_notes = check_outgoing_rog_limit(root, len(items))
    limit_warnings = [n for n in limit_notes if n.startswith("numerical_limit_county_local")]
    ...
    if not package.is_file():
        ...
        # Isolation: package must not embed Bates-like ROG-00N tokens
    ...
    for command in gates:
        ...
        if code != 0:
            ...
            return 1
    print("PASS: outgoing ROG validation")
    return 0
...
def _create_synthetic_matter(root: Path, matter_id: str, prefix: str) -> None:
    ...
    (root / "01_discovery_outgoing" / "rog_issue_brief.md").write_text(
        "# SYNTHETIC / NON-CLIENT / TEST ONLY\n\n"
        "- [notice] State all facts concerning defendant's receipt of any written complaint "
        "about the ladder on or before May 1, 2024. | Jury: prior notice\n"
        "- [wage_loss] Identify each period from June 2, 2024 through July 15, 2024 during "
        "which plaintiff was unable to work.\n"
        "- [liability] State all facts concerning defendant's duty to maintain the ladder AND "
        "identify each inspection of the ladder performed in 2024.\n"
        "- [jury_theme, authenticity] Describe the creation and custody of the photograph log "
        "marked as an exhibit. | Jury: exhibit authenticity\n",
        encoding="utf-8",
    )
...
def cmd_selftest(_args: argparse.Namespace) -> int:
    with tempfile.TemporaryDirectory(prefix="rog-outgoing-selftest-") as tmp:
        root = Path(tmp)
        a = root / "SYNTHETIC_client_a"
        b = root / "SYNTHETIC_client_b"
        _create_synthetic_matter(a, "SYN-OROG-A", "THORN-PROD")
        _create_synthetic_matter(b, "SYN-OROG-B", "RIVER-PROD")
        for matter in (a, b):
            for command in (
                ["parse-issue-brief", str(matter)],
                ["draft-outgoing-rog", str(matter)],
                ["package-outgoing-rog", str(matter)],
                ["validate-outgoing-rog", str(matter)],
            ):
                code = main(command)
                if code != 0:
        ...
        if "RIVER-PROD" in a_pkg or "THORN-PROD" in b_pkg:
            ...
            return 1
        print("PASS: rog-outgoing selftest")
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("parse-issue-brief", help="parse outgoing ROG issue brief")
    p.add_argument("matter_dir")
    p.add_argument("--source", type=Path)
    p.set_defaults(fn=cmd_parse_issue_brief)

    p = sub.add_parser("draft-outgoing-rog", help="draft narrow outgoing ROGs from targets")
    p.add_argument("matter_dir")
    p.set_defaults(fn=cmd_draft_outgoing_rog)

    p = sub.add_parser("package-outgoing-rog", help="write outgoing_rog_set.md")
    p.add_argument("matter_dir")
    p.set_defaults(fn=cmd_package_outgoing_rog)

    p = sub.add_parser("validate-outgoing-rog", help="run Slice B2 validators and gates")
    p.add_argument("matter_dir")
    p.add_argument("--skip-live-preflight", action="store_true")
    p.add_argument("--synthetic", action="store_true")
    p.set_defaults(fn=cmd_validate_outgoing_rog)

    p = sub.add_parser("selftest", help="offline synthetic outgoing ROG E2E")
